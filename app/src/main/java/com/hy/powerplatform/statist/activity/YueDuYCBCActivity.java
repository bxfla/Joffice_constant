...
//                Log.i("main", "response:" + response.body().string());
    ...
    public LineData getLineData() {
        ...
        for (int i = 0; i < beanList.size(); i++) {//y轴的数据
            float result = Float.parseFloat(String.valueOf(beanList.get(i).getCounts()));
            yList.add(new Entry(result, i));
        }
        LineDataSet lineDataSet = new LineDataSet(yList, getResources().getString(R.string.oaflow_statist_rb9));//y轴数据集合
